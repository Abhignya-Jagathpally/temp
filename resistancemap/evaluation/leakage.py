...
def audit_patient_split(
    ...
    Args:
        train_ids: Iterable of patient IDs assigned to the training split.
        val_ids: Iterable of patient IDs assigned to the validation split.
        test_ids: Iterable of patient IDs assigned to the test split.

    Returns:
        A :class:`LeakageReport`. ``clean`` is ``True`` only when none of
        the inputs were empty and there were zero overlaps.
    """
    train = _to_id_list(train_ids)
    val = _to_id_list(val_ids)
    test = _to_id_list(test_ids)
...
def detect_temporal_leakage(
    ...
    split_assignments: Mapping[Any, str],
