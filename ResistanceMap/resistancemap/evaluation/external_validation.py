...
class ExternalCohortLoader:
    """Real external validation cohort loader.

    Only MMRF CoMMpass has a working data connector in this build; the other
    four cohorts require consortium DUAs/IRB and are documented but not
    callable. Calling their loaders raises NotImplementedError with
    actionable next steps. ResistanceMap never fabricates external cohort
    data; the previous synthetic-fallback path was removed in v7.

    Note on sample sizes: AVAILABLE_COHORTS records the on-disk N for
    cohorts with a real connector; UNAVAILABLE_COHORTS records the
    published consortium N (for documentation only). The previous
    `COHORTS` dict listed MMRF_CoMMpass at n=1143; the on-disk GDC IA22
    dump under data/raw/mmrf_commpass/clinical.tsv actually contains 996
    submitter_ids — the 1143 figure was the consortium publication N, not
    the public GDC release N.
    """
    ...
    # Legacy attribute kept for backward compatibility with any caller still
    # referencing COHORTS. Reads as union of available + unavailable.
    COHORTS = {**AVAILABLE_COHORTS, **{k: {"n": v["n"], "description": v["reason"]}
                                       for k, v in UNAVAILABLE_COHORTS.items()}}

    def __init__(self, seed: int = 42):
        """Initialize loader. `seed` is retained only for API back-compat;
    ...
    def load_mmrf_compass(
        ...
        Requires that the MMRF block has been written into data_ready.pt
        by harmonize_omics (see P2.1 MMRF wiring). If the block is absent,
        raises a directive error telling the user how to populate it.
        """
        ...
        if not mmrf or "patient_ids" not in mmrf:
            raise RuntimeError(
                "MMRF block missing from data_ready.pt. The harmonize_omics "
                "hook in resistancemap/data/preprocessors.py is responsible "
                "for attaching ds.mmrf when data/raw/mmrf_commpass/clinical.tsv "
                "is present (see P2.1 wiring). Re-run prepare_data after "
                "wiring resistancemap/data/mmrf_loader.parse_gdc_clinical "
        ...
        return ExternalCohort(
            name="MMRF_CoMMpass",
            n_samples=n,
            features=np.full((n, 1), np.nan),  # to be filled by the predict step
            risk_scores=np.full(n, np.nan),    # to be filled by the predict step
    ...
    def _unavailable(self, name: str) -> ExternalCohort:
        ...
        raise NotImplementedError(
            ...
            "either obtain consortium access or remove the cohort from your "
            "evaluation manifest."
        )

    def load_gmmg_mm5(self) -> ExternalCohort:  # noqa: D401 - thin wrapper
...
class ExternalValidationOrchestrator:
    ...
    def tier3_multicenter_validation(
        ...
        NOTE (v12-audit-reports): Only 1 of 5 advertised external cohorts
        (MMRF_CoMMpass) currently has a real data connector wired in
        (see ExternalCohortLoader.AVAILABLE_COHORTS). Tier-3 therefore
        cannot succeed on real data with the current connectors —
        the other 4 cohorts raise NotImplementedError per the no-
        synthetic-data project policy. To enable Tier-3, wire data
        ingestion for at least one additional cohort from
        ExternalCohortLoader.UNAVAILABLE_COHORTS (requires consortium
        DUAs/IRB), then add it to AVAILABLE_COHORTS.
        ...
        if len(cohorts) < 2:
            raise ValueError(
                "Tier 3 requires ≥2 cohorts, but only 1 (MMRF_CoMMpass) has a "
                "real data connector in the current build. Either obtain "
                "consortium access for GMMG-MM5 / IFM-DFCI / PETHEMA-GEM / "
                "HOVON-65 and add to ExternalCohortLoader.AVAILABLE_COHORTS, "
                "or skip Tier 3 in your evaluation manifest."
            )
