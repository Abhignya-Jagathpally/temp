# Wave 4 — Master Head-to-Head: v11.5 vs Literature SOTA (W4.6)

**Author:** main thread (assembled from W4.2/W4.3/W4.4/W4.5 JSON outputs after agents hit Write/Bash sandboxes)
**Date:** 2026-05-07
**Eval protocol:** N=787 MMRF baseline, TT2L PFS, 224 events. Per-method evaluation: LOO Cox-equivalent (RSF) or 5-fold CV (DeepSurv, SCOPE — neural-net training too slow for LOO). Paired bootstrap B=10000 stratified by cyto vs `Cox_v11_routed_mmsygnal` (v11.5) per-patient log-hazards.

## §1 Master ranking table (all real LOO/CV runs on identical N=787 data)

| Model | Marginal C | S1 | S2 | S3 | S4 | S5 | Δ vs v11.5 | 95% CI | p | Verdict | Bonferroni m=6 |
|---|---:|---:|---:|---:|---:|---:|---:|---|---:|---|---|
| **v11.5 Cox_v11_routed_mmSYGNAL** | **0.6955** | 0.730 | 0.667 | 0.694 | 0.643 | 0.659 | ref | ref | ref | reference | — |
...
**On the same N=787 MMRF preprocessed data, with the same LOO/CV evaluation, with the same paired-bootstrap test against v11.5:**

- **mmSYGNAL routed (literature SOTA on MMRF) — TIES v11.5.** Paired bootstrap 99% CI excludes |Δ|>0.036 (Wave 1 reproduction). v11.5 ≈ mmSYGNAL on this cohort.
- **RSF (Ishwaran 2008) — LOSES v11.5** by Δ=−0.060 to −0.073 across both feature sets. p<0.001, survives Bonferroni.
- **DeepSurv (Katzman 2018) — LOSES v11.5** by Δ=−0.060 to −0.072. p<0.001, survives Bonferroni.
- **SCOPE / Hussain & Sontag 2024 transformer — LOSES v11.5** by Δ=−0.063 to −0.081. p<0.001, survives Bonferroni. Adding PCA factors makes SCOPE WORSE (more tokens → more transformer overfitting at N=787).
...
Out of 6 head-to-head comparisons against v11.5 with explicit paired bootstrap (the 4 implemented SOTA models × {v11_richer, v11_richer+PCA29}):

- **Beating v11.5 at any conventional α**: 0/6
- **Tying v11.5** (CI contains 0): 0/6 of the 6 SOTA-vs-v11.5 comparisons; 1 (mmSYGNAL) ties from the Wave-1 paired test
- **Losing v11.5 at α_bonf=0.0083**: 6/6
...
- v11.5 **beats** every off-the-shelf SOTA we implemented (RSF, DeepSurv, SCOPE) by Δ=+0.06 to +0.08 marginal C-index, p<0.001, all surviving Bonferroni m=6.
- v11.5 **ties** mmSYGNAL routed — and this is structural, not coincidental. **W3.6 empirically confirmed** that v11.5's entire +0.04 lift over Cox_v11_richer comes from the single engineered feature `mmsygnal_routed` (β·σ=+0.404, 2.6× the next-largest feature). The "tie with mmSYGNAL" is therefore a direct identity statement: v11.5 ≈ mmSYGNAL routing wrapped in a Cox model.

So the honest framing is **NOT** "v11.5 is a new architecture that beats SOTA." The honest framing is **"v11.5 = mmSYGNAL transcriptional program rules + a small set of clinical/cyto/Waddington features in a Cox PH wrapper. It ties mmSYGNAL on its own cohort and beats off-the-shelf RSF/DeepSurv/transformer baselines that lack the curated transcriptional-program signal."**

This validates two findings already in the audit record:
1. **Chair §3(ii):** v12 does not beat v11.5; v11.5 ties mmSYGNAL — confirmed at the per-implementation level here.
...
## §5 Where v11.5 wins, where it loses (per stratum vs the best alternative in each stratum)

| Stratum | v11.5 C | Best non-v11.5 C | Best non-v11.5 model | v11.5 win/loss |
|---|---:|---:|---|---|
| S1 del17p | 0.730 | 0.714 | RSF + PCA29 | **v11.5 wins +0.016** |
| S2 t(4;14) | 0.667 | 0.722 | mmSYGNAL routed | **v11.5 LOSES −0.055** (FGFR3+ submodel mmSYGNAL has) |
| S3 +1q21 | 0.694 | 0.671 | DeepSurv v11_richer | **v11.5 wins +0.023** |
| S4 t(11;14) | 0.643 | 0.693 | mmSYGNAL routed | **v11.5 LOSES −0.050** (chair §3(iii) safety flag in action) |
| S5 other | 0.659 | 0.682 | mmSYGNAL routed | **v11.5 LOSES −0.023** |

Per-stratum, **mmSYGNAL beats v11.5 in 3/5 strata** (S2 t(4;14), S4 t(11;14), S5 other) — but the marginal-C tie holds because S1+S3 advantages of v11.5 offset.
...
3. Per-stratum table (§5) makes it clear v11.5 is **not uniformly best** — the mmSYGNAL-routing dependency is what makes the marginal C high; without it, v11.5 ≤ DeepSurv in 3 strata.
4. The honest-framing sentence per chair §3(ii): "v11.5 ties mmSYGNAL on the same cohort and beats RSF/DeepSurv/SCOPE by Δ=+0.06 to +0.08, p<0.001, surviving Bonferroni m=6. v11.5's discriminative power is concentrated in the `mmsygnal_routed` feature (β·σ=+0.404, 2.6× the next feature; W3.6 §A.1)."
