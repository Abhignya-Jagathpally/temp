...
==================================
Train MORT-FM Stage E (longitudinal trajectory).

Requires temporal pairs with ``x_t_delta is not None`` for a usable fraction
of rows; otherwise the trajectory loss receives no supervision.
"""
...
logger = logging.getLogger("mortfm_train_trajectory")


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--config", default="configs/mortfm_trajectory.yaml")
    ap.add_argument("--pairs", required=True)
    ap.add_argument("--init-from", default=None, help="Optional pretrained checkpoint")
    ...
    logger.info("Trajectory pairs (x_t_delta present): %d / %d", len(longitudinal), len(pairs))
    if len(longitudinal) < cfg.min_paired_n_for_longitudinal_claim:
        logger.warning(
            "Only %d longitudinal pairs available; below threshold %d. "
            "Training will proceed but no longitudinal claim can be released.",
            len(longitudinal), cfg.min_paired_n_for_longitudinal_claim,
        )
    ...
    trainer.fit_stage("E", n_epochs=cfg.trajectory_epochs)
    trainer.save_checkpoint("stage_E_complete.pt")
    return 0
